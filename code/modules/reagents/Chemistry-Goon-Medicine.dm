...
/datum/reagent/strange_reagent/reaction_mob(var/mob/living/carbon/human/M as mob, var/method=TOUCH, var/volume)
	if(M.bruteloss > 80 || M.fireloss > 80)
		...
		var/mob/dead/observer/mob_ghost
		for(var/mob/dead/observer/ghost in player_list)
			if(ghost.mind == M.mind)
				mob_ghost = ghost
				ghost << 'sound/effects/adminhelp.ogg'
				ghost << "<b><font color = #330033><font size = 3>Your corpse is being revived. Return to your body if you want to be resurrected!</b> (Verbs -> Ghost -> Re-enter corpse)</font color>"
				break
		M.visible_message("<span class='warning'>[M]'s body convulses a bit.</span>")
		if(M.health <= config.health_threshold_dead && !M.suiciding && !mob_ghost && !(NOCLONE in M.mutations))
			M.stat = 1
			...
			M.emote("gasp")
			message_admins("[M.name]/([M.ckey]) was revived using the strange reagent (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[M.x];Y=[M.y];Z=[M.z]'>JMP</a>)")
			M.zombify()
	..()
	return
